...
export default async function PresupuestoPage({ searchParams }: PageProps) {
  ...
  const totalEventos   = sum(ppto.map((r: any) => r.eventos ?? 0))
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12, marginBottom: 24 }}>
        ...
        <KpiBox label="Online" value={soles(totalOnline, 0)} subvalue="canal digital" />
        <KpiBox label="Offline" value={soles(totalOffline, 0)} subvalue="canal presencial" />
        <KpiBox label="Eventos" value={soles(totalEventos, 0)} />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        ...
        <GlassCard title="Desglose Online / Offline / Eventos">
          {ppto.length > 0 ? (
            <Suspense fallback={<div style={{ height: 280 }} />}>
              <PlotlyChart height={280} data={[
                ...
                { type: 'bar', name: 'Eventos', x: meses, y: ppto.map((r: any) => r.eventos), marker: { color: '#7C3AED' } },
